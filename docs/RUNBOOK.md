...
- Standard console streams: Neither the application nor the API writes log files to disk. All log messages, warnings, and error tracebacks print directly to `stdout` and `stderr` in the launching terminal.
- Capturing console logs: To save logs to a file, redirect terminal output in PowerShell:

  ```powershell
  uv run streamlit run app.py --server.port 8841 2>&1 | Tee-Object -FilePath app.log
  ```

- In-app diagnostics: The Streamlit Diagnostics page (`app_pages/diagnostics.py`) shows model readiness, stage durations, bottlenecks, and page-level warnings.
- User downloads: Files are written to disk only when requested via browser download buttons or the API artifact download endpoints (`/api/v1/jobs/{job_id}/artifacts/{artifact_name}`).
...
| `HTTP 409 parse_unavailable` | Schema extraction (`POST /api/v1/jobs/{job_id}/extract`) was called before Parse processing succeeded. | Confirm the document is parsed successfully before requesting schema extraction. |
