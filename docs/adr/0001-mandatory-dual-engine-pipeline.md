...
# ADR 0001: Require both extraction engines
...
RapidOCR supplies local text, confidence, and geometry. GPT supplies semantic
...
document content. After that preflight succeeds, RapidOCR produces the local
first pass before any GPT refinement request. Every GPT request uses
`gpt-5.6-luna` with medium reasoning effort. Missing or invalid OpenAI
configuration, RapidOCR initialization or all-page OCR failure, and required
...
- Balanced mode may reduce transmitted OCR context and cost, but still sends
  every selected page image and makes at least one GPT call.
...
- UI, API, manifests, and documentation describe only the mandatory dual-engine
  workflow.
